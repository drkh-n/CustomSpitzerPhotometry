pro test_image, im, imname, rap, rbackin, rbackout, apcor, ra_decimaldeg, dec_decimaldeg, $
  ...
  place_prf, im_test, channel, x_pos, y_pos, x_pos-fix(x_pos), $
             y_pos-fix(y_pos), psf, factor, imout

  ;--- photometry
  circapphot, imout, x_pos, y_pos, rap, tot, imag, 1., npix, $
              pixsig, magerr, rbackin=rbackout, bgndwidth=$
  ...
  result = {name:imname,ra:ra_decimaldeg,dec:dec_decimaldeg,ch:channel, $
            ...
            phot:8.47*apcor[channel-1]*tot, sigma:8.47*apcor[channel-1]*sigma}
